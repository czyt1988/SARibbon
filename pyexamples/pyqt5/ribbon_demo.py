"""
SARibbon Python Demo (PyQt5)
============================

A self-contained demo showing how to use SARibbon via PyQt5 Python bindings.
...
Prerequisites:
  pip install PyQt5
  Build and install PyQtSARibbon first (see tools/build_python_bindings.bat)
...
def main():
    ...
    win.setWindowTitle("SARibbon Python Demo (PyQt5)")
    win.resize(1280, 800)
    ...
    # QButtonGroup.buttonClicked(int) fires when a button is selected.
    # The int is the style enum value we passed to addButton().
    style_button_group.buttonClicked[int].connect(
    ...
    editor.setPlaceholderText("SARibbon Python Demo (PyQt5) - type here...")
    ...
    ribbon.actionTriggered.connect(
        lambda act: print("[PyQt5] Action: {}".format(act.text()))
    )
