#ifndef SARIBBONVERSIONINFO_H
#define SARIBBONVERSIONINFO_H
/**
 * \if ENGLISH
 * @file This file is automatically generated by cmake from SARibbonBarVersionInfo.h.in, any changes to this file will be overwritten
 * \endif
 *
 * \if CHINESE
 * @file 此文档由cmake根据SARibbonBarVersionInfo.h.in自动生成，任何在此文件上的改动都将覆写
 * \endif
 */

/**
 * \if ENGLISH
 * @def Major version number of SARibbonBar
 * \endif
 *
 * \if CHINESE
 * @def ribbon的数字版本 {MAJ}.MIN.PAT
 * \endif
 */
#ifndef SA_RIBBON_BAR_VERSION_MAJ
#define SA_RIBBON_BAR_VERSION_MAJ 2
#endif
/**
 * \if ENGLISH
 * @def Minor version number of SARibbonBar
 * \endif
 *
 * \if CHINESE
 * @def ribbon的数字版本 MAJ.{MIN}.PAT
 * \endif
 */
#ifndef SA_RIBBON_BAR_VERSION_MIN
#define SA_RIBBON_BAR_VERSION_MIN 6
#endif
/**
 * \if ENGLISH
 * @def Patch version number of SARibbonBar
 * \endif
 *
 * \if CHINESE
 * @def ribbon的数字版本 MAJ.MIN.{PAT}
 * \endif
 */
#ifndef SA_RIBBON_BAR_VERSION_PAT
#define SA_RIBBON_BAR_VERSION_PAT 1
#endif

/**
 * \if ENGLISH
 * @def Version string of SARibbonBar
 * \endif
 *
 * \if CHINESE
 * @def 版本号（字符串）
 * \endif
 */
#ifndef SARIBBON_VERSION
#define SARIBBON_VERSION "2.6.1"
#endif

#endif // SARIBBONVERSIONINFO_H
