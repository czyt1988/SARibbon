...
确保 Qt 已正确安装并配置环境变量。你可以通过以下方式检查：
...
**错误信息**：Cannot run compiler 'cl' (Windows) 或 compiler not found

**解决方案**：确保在正确的开发环境中运行构建命令。Windows 用户需要在 Visual Studio 开发者命令提示符中执行。

### 问题 3：DLL 加载失败

**错误信息**：ImportError: DLL load failed while importing PyQtSARibbon

**解决方案**：将 Qt 的 DLL 复制到 Python 的 site-packages 目录：
...
**错误信息**：ImportError: Module use of python3x.dll conflicts with this version of Python

**解决方案**：确保 PyQt5、sip 和 Python 版本兼容。建议使用相同来源的 PyQt5 和 Python。
...
``bash
# Windows
del <Python路径>\Lib\site-packages\PyQtSARibbon.pyd

# Linux/macOS
rm <Python路径>/lib/python3.x/site-packages/PyQtSARibbon.so
``

## 下一步

构建完成后，请参阅 [使用 Python 绑定](./use-python-bindings.md) 了解如何在 Python 中使用 SARibbon。
